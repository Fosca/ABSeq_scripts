...
for ttype in ['habituation', 'standard', 'violation']:
    # Open figure
    plt.close('all')
    fig, ax = plt.subplots(3, 1, figsize=figsize, sharex=False, constrained_layout=True)  #, sharex=False, sharey=True, constrained_layout=True)
    fig.suptitle('Corr_GFPxComplexity: ' + ttype + ' trials', fontsize=12)

    for iplot, ch_type in enumerate(['eeg', 'mag', 'grad']):
