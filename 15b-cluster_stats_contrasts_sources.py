...
if resid_epochs:
    results_path = op.join(config.result_path, 'Contrasts_tests', resid_epochs_type, analysis_main_name, 'Sources')
else:
    results_path = op.join(config.result_path, 'Contrasts_tests', analysis_main_name,  'Sources')
...
for analysis_name in [analysis_main_name + '_allseq', analysis_main_name + '_seqID1', analysis_main_name + '_seqID2', analysis_main_name + '_seqID3', analysis_main_name + '_seqID4',analysis_main_name + '_seqID5', analysis_main_name + '_seqID6', analysis_main_name + '_seqID7']:
    ...
    for subject in subjects_list:
        ...
        if resid_epochs:
            evoked1, stc1 = source_estimation_funcs.load_evoked_with_sources(subject, evoked_filter_name=analysis_name + '_cond1', evoked_filter_not=None, evoked_path='evoked_resid', apply_baseline=True)
            evoked2, stc2 = source_estimation_funcs.load_evoked_with_sources(subject, evoked_filter_name=analysis_name + '_cond2', evoked_filter_not=None, evoked_path='evoked_resid', apply_baseline=True)
        else:
            evoked1, stc1 = source_estimation_funcs.load_evoked_with_sources(subject, evoked_filter_name=analysis_name + '_cond1', evoked_filter_not=None, evoked_path='evoked_cleaned', apply_baseline=True)
            evoked2, stc2 = source_estimation_funcs.load_evoked_with_sources(subject, evoked_filter_name=analysis_name + '_cond2', evoked_filter_not=None, evoked_path='evoked_cleaned', apply_baseline=True)
    ...
    if RunStats:
        ...
        time_window = [0.000, 0.600]  # to crop data on the time dimension
        ...
        T_obs, clusters, cluster_p_values, H0 = clu = spatio_temporal_cluster_1samp_test(X, connectivity=connectivity, n_jobs=6, n_permutations=500, threshold=t_threshold, buffer_size=None, verbose=True)  # with connectivity instead of adjacency
        ...
        for ii, iclu in enumerate(good_cluster_inds):
            if ii <= 4:
                ...
                brain = cluster_stc.plot(hemi='split', views=['lat', 'med'], subjects_dir=fsMRI_dir, size=(800, 800), smoothing_steps=5,
                                         clim=dict(kind='value', pos_lims=[datamin, datamin+(datamax-datamin)/2, datamax]), initial_time=timeval, time_viewer=False)  # , show_traces=True)
                brain.save_image(op.join(results_path, 'Sources_' + analysis_name + '_cluster_' + str(ii+1) + '_' + str('%d' % (timeval * 1000)) + 'ms.png'))
                brain.close()


# =============================================================================================
# ========= TEST EXTRACT ROI DATA
subjects_list = config.subjects_list[0:6]
# for analysis_name in [analysis_main_name + '_allseq', analysis_main_name + '_seqID1', analysis_main_name + '_seqID2', analysis_main_name + '_seqID3', analysis_main_name + '_seqID4',analysis_main_name + '_seqID5', analysis_main_name + '_seqID6', analysis_main_name + '_seqID7']:
for analysis_name in [analysis_main_name + '_seqID1']:
    ...
    for subject in subjects_list:
        # Load evoked and sources for the 2 conditions
        if resid_epochs:
            evoked1, stc1 = source_estimation_funcs.load_evoked_with_sources(subject, evoked_filter_name=analysis_name + '_cond1', evoked_filter_not=None, evoked_path='evoked_resid', apply_baseline=True, morph_sources=False)
            evoked2, stc2 = source_estimation_funcs.load_evoked_with_sources(subject, evoked_filter_name=analysis_name + '_cond2', evoked_filter_not=None, evoked_path='evoked_resid', apply_baseline=True, morph_sources=False)
        else:
            evoked1, stc1 = source_estimation_funcs.load_evoked_with_sources(subject, evoked_filter_name=analysis_name + '_cond1', evoked_filter_not=None, evoked_path='evoked_cleaned', apply_baseline=True, morph_sources=False)
            evoked2, stc2 = source_estimation_funcs.load_evoked_with_sources(subject, evoked_filter_name=analysis_name + '_cond2', evoked_filter_not=None, evoked_path='evoked_cleaned', apply_baseline=True, morph_sources=False)
