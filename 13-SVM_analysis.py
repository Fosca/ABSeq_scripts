...
for subject in config.subjects_list:
    ...
    GAT_results = np.load(op.join(SVM_path, suf+'GAT_results'+score_suff+'.npy'), allow_pickle=True).item()
    print(op.join(SVM_path, suf+'GAT_results'+score_suff+'.npy'))
    ...
    for sens in ['eeg', 'mag', 'grad','all_chans']:
        ...
        for key in ['SeqID_%i' % i for i in range(1, 8)]:
            ...
            #SVM_funcs.plot_GAT_SVM(GAT_results[sens][key], times, sens=sens, save_path=sub_fig_path, figname=suf+'GAT_' + key +score_suff+ '_'); plt.close('all')
        ...
        SVM_funcs.plot_GAT_SVM(GAT_results[sens]['average_all_sequences'], times, sens=sens, save_path=sub_fig_path, figname=suf+'GAT_all_seq'+score_suff+'_',vmin=0.4,vmax=0.6)
        plt.close('all')
