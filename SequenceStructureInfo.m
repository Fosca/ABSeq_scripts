...
% Ex: [[[AA][BB]][AAA]]
% /!\ The whole sequence is within brackets so the structure opens at the
% first item and closes at the last item /!\
...
% data.Sequence = {'AABBAAA'}
% data.SequenceBrackets = {'[[[AA][BB]][AAA]]'}
for nseq=1:numel(data.Sequence)
    ...
    data.ChunkDepth.(seq) = data.ChunkDepth.(seq) - 1; % since the whole sequence is within []
