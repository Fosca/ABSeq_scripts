...
for ii, name in enumerate(chances.keys()):
    ...
    SVM_funcs.plot_gat_simple(anal_name, config.subjects_list, '/feature_decoding/' + name + '/perf_',score_field='score',
                              chance=chances[name],compute_significance=None, plot_per_subjects=True,vmin=None,vmax=None)

# ======================================= plotting the linear regression results =============================================

for name in ['OpenedChunks','ClosedChunks','ChunkDepth']:
    anal_name = 'feature_decoding/' + prefix + name + suffix
    SVM_funcs.plot_gat_simple(anal_name, config.subjects_list, '/feature_decoding/'+name+'/r_', chance=0, score_field='regression',
